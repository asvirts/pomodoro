// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'settings_provider.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$fontNotifierHash() => r'01f93dea6f71b7f0b46d8e1094f448b41fd2f51a';

/// See also [FontNotifier].
@ProviderFor(FontNotifier)
final fontNotifierProvider =
    AutoDisposeNotifierProvider<FontNotifier, Set<Fonts>>.internal(
  FontNotifier.new,
  name: r'fontNotifierProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$fontNotifierHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$FontNotifier = AutoDisposeNotifier<Set<Fonts>>;
String _$selectedFontIndexHash() => r'4c40cd1a546610aaf8fd7fd040e53e99295c5383';

/// See also [SelectedFontIndex].
@ProviderFor(SelectedFontIndex)
final selectedFontIndexProvider =
    AutoDisposeNotifierProvider<SelectedFontIndex, Set<int>>.internal(
  SelectedFontIndex.new,
  name: r'selectedFontIndexProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$selectedFontIndexHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$SelectedFontIndex = AutoDisposeNotifier<Set<int>>;
String _$colorsNotifierHash() => r'126b76fcb28261d92e1d6bd7f575a0b555cf56b9';

/// See also [ColorsNotifier].
@ProviderFor(ColorsNotifier)
final colorsNotifierProvider =
    AutoDisposeNotifierProvider<ColorsNotifier, Set<PomodoroColors>>.internal(
  ColorsNotifier.new,
  name: r'colorsNotifierProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$colorsNotifierHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$ColorsNotifier = AutoDisposeNotifier<Set<PomodoroColors>>;
String _$selectedColorHash() => r'9151b791467f877c853d124dae1acf6b66845e01';

/// See also [SelectedColor].
@ProviderFor(SelectedColor)
final selectedColorProvider =
    AutoDisposeNotifierProvider<SelectedColor, Set<int>>.internal(
  SelectedColor.new,
  name: r'selectedColorProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$selectedColorHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$SelectedColor = AutoDisposeNotifier<Set<int>>;
String _$colorRedSelectedHash() => r'6998bafb60383ce70b77227afc2b4ec9c579311c';

/// See also [ColorRedSelected].
@ProviderFor(ColorRedSelected)
final colorRedSelectedProvider =
    AutoDisposeNotifierProvider<ColorRedSelected, Set<bool>>.internal(
  ColorRedSelected.new,
  name: r'colorRedSelectedProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$colorRedSelectedHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$ColorRedSelected = AutoDisposeNotifier<Set<bool>>;
String _$colorBlueSelectedHash() => r'84791f3112c78e8fa4166fd27482948f664a3713';

/// See also [ColorBlueSelected].
@ProviderFor(ColorBlueSelected)
final colorBlueSelectedProvider =
    AutoDisposeNotifierProvider<ColorBlueSelected, Set<bool>>.internal(
  ColorBlueSelected.new,
  name: r'colorBlueSelectedProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$colorBlueSelectedHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$ColorBlueSelected = AutoDisposeNotifier<Set<bool>>;
String _$colorPurpleSelectedHash() =>
    r'82dfbf19d220de24c6e8a8bbbe3be890aa7708de';

/// See also [ColorPurpleSelected].
@ProviderFor(ColorPurpleSelected)
final colorPurpleSelectedProvider =
    AutoDisposeNotifierProvider<ColorPurpleSelected, Set<bool>>.internal(
  ColorPurpleSelected.new,
  name: r'colorPurpleSelectedProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$colorPurpleSelectedHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$ColorPurpleSelected = AutoDisposeNotifier<Set<bool>>;
String _$pomodoroTimerNotifierHash() =>
    r'a036eee72177e38c409911f6dee55552972db4eb';

/// See also [PomodoroTimerNotifier].
@ProviderFor(PomodoroTimerNotifier)
final pomodoroTimerNotifierProvider =
    AutoDisposeNotifierProvider<PomodoroTimerNotifier, Set<int>>.internal(
  PomodoroTimerNotifier.new,
  name: r'pomodoroTimerNotifierProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$pomodoroTimerNotifierHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$PomodoroTimerNotifier = AutoDisposeNotifier<Set<int>>;
String _$shortBreakTimerNotifierHash() =>
    r'27504a7fc42b77fd127e132a15a56564241dc00d';

/// See also [ShortBreakTimerNotifier].
@ProviderFor(ShortBreakTimerNotifier)
final shortBreakTimerNotifierProvider =
    AutoDisposeNotifierProvider<ShortBreakTimerNotifier, Set<int>>.internal(
  ShortBreakTimerNotifier.new,
  name: r'shortBreakTimerNotifierProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$shortBreakTimerNotifierHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$ShortBreakTimerNotifier = AutoDisposeNotifier<Set<int>>;
String _$longBreakTimerNotifierHash() =>
    r'73178a14949b4112e18776b8ab160fd76a73dd0e';

/// See also [LongBreakTimerNotifier].
@ProviderFor(LongBreakTimerNotifier)
final longBreakTimerNotifierProvider =
    AutoDisposeNotifierProvider<LongBreakTimerNotifier, Set<int>>.internal(
  LongBreakTimerNotifier.new,
  name: r'longBreakTimerNotifierProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$longBreakTimerNotifierHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$LongBreakTimerNotifier = AutoDisposeNotifier<Set<int>>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
